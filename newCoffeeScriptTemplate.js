...
(function() {
  var addHandlersToSerials, animBtnAppear, changeLanguage, createInput, createSerial, createUserIDHidden, createUserSerials, deleteUserIDHidden, emptyGreeting, greeting, hideButtons, hideLoader, isNumeric, islteIE8, scrollToTop, showLoader, slideContext, translate, updateHeightPage;
  ...
  changeLanguage = function(lang) {
    switch (lang) {
      case "ru":
        ...
        if (islteIE8()) {
          $(".trig").css({
            filter: "progid:DXImageTransform.Microsoft.AlphaImageLoader(src=\"../img/trig-ru_cr.png\", sizingMethod=\"scale\")"
          });
        } else {
          $(".trig").css({
            backgroundImage: "url(img/trig-ru_cr.png)"
          });
        }
  ...
  islteIE8 = function() {
    var isIE, isIE9orBigger;
    isIE = document.querySelector || window.XMLHttpRequest || document.compatMode;
    isIE9orBigger = document.addEventListener;
    return isIE && !isIE9orBigger;
  };

  hideButtons = function(isDemo) {
    if (isDemo) {
      $("#btn-logout").css({
        display: "none"
      });
  ...
  createInput = function(name, type, _class, value) {
    value = (value ? " value=\"" + value + "\"" : "");
    return "<input name=\"" + name + "\" type=\"" + type + "\" class=\"" + _class + "\"" + value + " >";
  };

  createSerial = function(serialData, isNew) {
    var hidden, listItem, res;
    hidden = "";
    if (isNew) {
  ...
  createUserSerials = function(data, intoEl) {
    var userData, x;
  ...
  addHandlersToSerials = function(el) {
    var addNumber, getEventTarget;
    addNumber = function(elem, number) {
      var resValue;
  ...
  updateHeightPage = function() {
    var addedPixels, contextDefault, defaultHeight, el, heightEl, maxCapacityElements, menuDefault, numberSerialExists, tableDefault;
  ...
  $(function() {
    var addErrorMessage, checkRegisterData, contextDiv, createDefaultSerialsData, hideMessageBoxes, passEncr, realFalse, removeErrorMessages, slideContext, slideContextAndIndex, slideContextDiv, slideIndexDiv;
    createDefaultSerialsData = function(count) {
      var data, serialData, userData, x;
      data = [];
      userData = [];
      x = 0;
      while (x < count) {
    ...
    addErrorMessage = function(message, el, border) {
      var div;
    ...
    removeErrorMessages = function(border) {
      var countMessages, errors;
      errors = border.find(".error-message");
      countMessages = errors.length;
      if (countMessages > 0) {
        errors.remove();
        border.css({
          height: "-=" + 34 * countMessages + "px"
        });
      }
    };
    realFalse = function(el) {
      return el !== undefined && el === false;
    };
    checkRegisterData = function(data) {
      var borderRegister, res;
      ...
      if (data["login"] === undefined || data["pass"] === undefined || data["captcha"] === undefined) {
        res = false;
      }
      return res;
    };
    passEncr = function(passKey, sting) {
      var indexAmpersand, indexPassKey, indexStartValue, newString, value;
    ...
    hideMessageBoxes = function() {
      $("#backgroundShadow,#border-register,#border-login").css({
        display: "none"
      });
    };
    slideContextAndIndex = function(indexPosition, contextPosition, firstIndexAnimate, complete) {
      if (firstIndexAnimate) {
        $("#global-index-div").css({
          position: "absolute"
        }).animate({
          left: indexPosition
        }, {
    ...
    slideContextDiv = function(complete) {
      slideContextAndIndex("-100%", "0%", true, complete);
    };
    slideIndexDiv = function(complete) {
    ...
    $("#loginform").submit(function(e) {
      var checkLoginData, postData;
      checkLoginData = function(data) {
        var res;
    ...
    $("#btn-register").click(function() {
      scrollToTop();
      $("#backgroundShadow,#border-register").css({
        display: "block"
      });
    });
    $("#btn-login").click(function() {
      scrollToTop();
      $("#backgroundShadow, #border-login").css({
        display: "block"
      });
    });
    ...
    $("#btn-create").click(function() {
      var data, el, heightEl, lastSerialPosition, serialNotVisible;
      data = [];
      ...
      serialNotVisible = lastSerialPosition < $("html").scrollTop() || lastSerialPosition > $("html").scrollTop() + $(window).height();
      if (serialNotVisible) {
        $("html").animate({
    ...
    $("#btn-save").click(function() {
      var curSerialDiv, errorMessagePopup, incorrectEpisode, incorrectSeason, objData, serial, serials, serialsDivs, x;
      errorMessagePopup = function(el, message) {
        var div, errMessage, offset, popup, popupNotVisible, topPopupCoor;
        ...
        popupNotVisible = offset.top < $("html").scrollTop() || offset.top > $("html").scrollTop() + $(window).height();
        if (popupNotVisible) {
          $("html").animate({
      ...
      while (x < serialsDivs.length) {
        ...
        incorrectSeason = !isNumeric(serial.season);
